...
export default (
  ...
  logInWindow.webContents.on(`will-redirect`, async (event) => {
    ...
    if (
      ...
      if (!isProduction()) {
        log(`Skipping redirect to browser - development environment`);
      } else {
        log(`User is logging in, sending to browser for Google SSO`);
    ...
    if (url.startsWith(`${getSiteUrl()}/office/`)) {
      ...
      // URL fails to load.
      log(`Setting state.logInFlowCompleted = true`);
